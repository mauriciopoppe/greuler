...
describe('greuler', function () {
  describe('graph', function () {
    ...
    beforeEach(function () {
      ...
      manager = new GraphManager({ options: {} }, data)
      nEdges = data.edges.length
      nNodes = data.nodes.length
    })

    it('should have a constructor', function () {
      assert(new GraphManager(undefined, data))
    })
    ...
    describe('nodes', function () {
      ...
      it('should get nodes by fn', function () {
        const nodes = manager.getNodesByFn(function (v) {
          return v.id > 5
        })
        assert(nodes.length === 5)
        assert(
          !manager.getNodesByFn(function (v) {
            return v.id === 11
          }).length
        )
      })
      ...
      it('should get adjacent nodes', function () {
        const nodes = manager.getAdjacentNodes({ id: 1 })
    ...
    describe('edges', function () {
      ...
      it('should remove multiple edges', function () {
        const edges = manager.getIncidentEdges({ id: 1 })
