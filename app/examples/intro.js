  ...
  gen.run(function *algorithm(instance) {
    ...
    yield function () {
      ...
      instance.graph.removeEdges(
        instance.graph.getEdgesBetween(8, 1)
      );
      instance.update();
    };
    ...
    function *innerDfs(source) {
      ...
      function *dfs(u, p) {
        ...
        yield function () {
          instance.graph.getNode(u).topRight = timeIn[u] + '/' + low[u];
        ...
        for (var i = 0; i < adjacent.length; i += 1) {
          ...
          if (timeIn[v] === -1) {
            yield function () {
              instance.selector.traverseEdgesBetween({ source: u, target: v});
            };
            yield *dfs(v, u);
            ...
            yield function () {
              instance.selector.traverseEdgesBetween({ source: u, target: v, stroke: 'cyan' });
            };
    ...
    yield function () {
      updateText('that\'s it! thanks for watching :)');
    };
  });
})();
